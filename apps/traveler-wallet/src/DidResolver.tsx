// In-app DID resolver: resolves a did:cid via the wallet's keymaster and formats the
// document for demo & discussion. Shown as the wallet's "Resolver" tab; the resolve-icon
// next to any DID brings that DID here.
...
export function DidResolver({ km, target, onResolve }: { km: Keymaster; target: string | null; onResolve: (did: string) => void }) {
  ...
  const resolve = async (d: string) => {
    ...
    try { setDid(t); setDoc(await km.resolveDID(t)); }
